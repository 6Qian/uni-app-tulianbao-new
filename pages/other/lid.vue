<template>
	<view class="content">
		...
		<view class="ckm">
			...
			<view class="tt">出售后将冻结对应脉点积分</view>
			<view class="mon">可得<text>¥{{ ((num?num:0) * (row.money?row.money:0)).toFixed(2) }}</text></view>
		</view>
		<view class="info">
			<view class="title">收款信息</view>
			<view class="payment" v-if="!info.is_payment">
				<view class="tt">你还为绑定收款信息</view>
				<view class="aru" @click="ToPayment()">立即绑定</view>
			</view>
			<view class="paymentlist" v-else>
				<view :class="'lis '+(type.includes('alipay') ? 'active' : '')" @click="payment('alipay')" v-if="info.is_alipay">支付宝</view>
				<view :class="'lis '+(type.includes('wechat') ? 'active' : '')" @click="payment('wechat')"  v-if="info.is_wechat">微信</view>
				<view :class="'lis '+(type.includes('bank') ? 'active' : '')" @click="payment('bank')" v-if="info.is_bank">银行卡</view>
			</view>
			<!-- <view class="payments">
				<view class="icon">
					<image src="../../static/img/alipay.png" mode="aspectFill"></image>
				</view>
				<view class="ts">小白猿</view>
				<view class="tt">176*****8666</view>
				<view class="aru">修改信息</view>
			</view> -->
			...
			<view class="tips">交易将产生1%的手续费由卖方承担，订单生成后就立即扣除，交易失败则退回。</view>
		</view>
		
		<view class="pobtn" @click="join_sell()">
			立即出售脉点
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			ToOrder(id){
				uni.redirectTo({
					url:"/pages/index/between_order?id="+id
				})
			}
		},
	};
