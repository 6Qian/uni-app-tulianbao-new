<template>
	<view class="content">
		...
		<view class="center">
			...
			<view class="btn" @click="go">立即赠送</view>
			<view class="title" style="font-size: 28rpx;">为了您的积分安全，请谨慎赠送</view>
		...
		<view class="giveawaypopupcenter" v-if="giveaway">
			...
			<view class="pctt">转赠后无法撤回，请谨慎操作!</view>
			<view class="pcinput">
				<input type="text" v-model="uid" placeholder="请输入你要赠送的用户ID">
...
<script>
	export default {
		...
		methods: {
			go(){
				if(!this.user.check){
					this.app.toast('请选择接受用户');
					return;
				}
				if(!this.num){
					this.app.toast('请输入转赠数量');
					return;
				}
				this.app.confirm({
					content: "赠送结果不可逆，确认赠送吗？",
					confirmText: "确认",
					success:()=>{
						this.app.ajax({
							url: "/user/giveConfirm",
							data: { uid: this.uid,score:this.num },
							method: "post",
							success:(res)=> {
								if(res.code === 200){
									this.app.alert({
