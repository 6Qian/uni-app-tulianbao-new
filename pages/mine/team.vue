<template>
	<view class="content">
		<view :class="header_class" :style="{paddingTop:statusBarHeight+'px'}">
			...
			<!-- <text @click="bindlast" style="color: #ffffff; z-index: 3; position: absolute; top: 50%; transform: translateY(-50%); right: 30rpx;">绑定上级</text> -->
			<text @click="bindlast"
				style="color: #ffffff; z-index: 3; position: absolute; bottom: 12px; right: 30rpx;">绑定上级</text>
		</view>
		<view class="tops" style="z-index: 10;">
			...
			<view class="invdata" style="position: relative;">
				...
				<view class="lis" style="position: relative;" @click="ToMoney()">
					<view class="money"></view>
					<view class="tt" style="margin-top: 16rpx;">邀请奖励</view>
				</view>
		...
		<view class="giveawaypopupcenter" v-if="giveaway">
			<view class="pcts">
				团队信息
				<view class="text" style="font-size: 14px;display: inline-block;">（ 上级id: {{info.p_userid || '0'}}
					上级用户名:{{info.p_username || '无'}}）</view>
			</view>
		...
		methods: {
			...
			bindlast() {
				uni.showModal({
					title: '绑定上级',
					editable: true,
					placeholderText: '请输入上级id',
					success: (res) => {
						if (res.confirm) {
							if (!res.content.trim().length) return;
							this.app.ajax({
								url: "/user/upbindperson",
								method: "post",
								data: {
									pid: res.content,
								},
